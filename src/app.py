...
actual_values = np.repeat(np.array([275, 280])[:, np.newaxis], d, axis=1) # Actual GHG inventory values for 2020 and 2021
V2 = np.mean([260, 300]) # Starting value for 2022
#dB = 40 * np.random.normal(size=(n - 1 - 1, d)) # random walk # 30 is heuristic as an annual absolute yearly growth # -1 is for the two initial years but the first one is removed
dB = 30 * np.random.uniform(low=-1, high=1, size=(n - 1 - 1, d)) #using a uniform distribution instead of normal
...
B0 = np.full(shape=(1, d), fill_value=V2) # initial value for 2022
B = np.cumsum(np.concatenate((B0,dB), axis=0),axis=0) # accumulated sum of random walk with initia value B0
B = np.concatenate((actual_values,B[1:,]), axis=0) #Actual values are concatenated to B where its first year is removed
